...
def fetch_collection(user, token):
    ...
    for attempt in range(12):
        ...
        if code == "200" and out.stat().st_size > 5000:
            print(f"collection.xml {out.stat().st_size} bytes")
            return out
...
def fetch_things(coll, token):
    ids = [i.get("objectid") for i in ET.parse(coll).getroot().findall("item")]
    print(f"{len(ids)} owned games")
    for i in range(0, len(ids), BATCH):
        chunk = ids[i:i + BATCH]
        out = RAW / f"thing_{i:04d}.xml"
        if out.exists() and out.stat().st_size > 5000 and not FORCE:
...
def main():
    ...
    c = load_creds()
    coll = fetch_collection(c["BGG_USERNAME"], c["BGG_TOKEN"])
    fetch_things(coll, c["BGG_TOKEN"])
